...
int exein_fetch_data(exein_shandle *uhandle, uint16_t pid, uint16_t *dstbuf){
	...
	pthread_mutex_lock(&uhandle->data_avail);
	for(buf=uhandle->buffers; buf != NULL; buf=(exein_buffers *)(buf->hh.next)) {
			if (buf->pid==pid){
#ifdef DEBUG
				printf("fetch Mutex LOCK\n");
#endif
...
int receive_feeds(void *data){
	...
	((proc_args *) data)->loading_done=	1;
	if (pthread_mutex_init(&uhandle->data_avail, NULL) != 0) {
		printf("mutex data_avail init failed\n");
	...
	while (1){
		...
		pthread_mutex_trylock(&uhandle->data_avail);
#ifdef DEBUG
		printf("Attempt to unlock already locked data_avail\n");
#endif
		pthread_mutex_unlock(&uhandle->data_avail);
...
void exein_agent_stop(exein_shandle *uhandle){
	...
	pthread_mutex_destroy(&uhandle->data_avail);
...
exein_shandle *exein_agent_start(uint32_t key, uint16_t tag)
{
	...
	int 			err;
        struct sigaction        sa;
#ifdef DEBUG
	printf("libexnl.exein_agent_start - staring up\n");
	printf("libexnl.exein_agent_start - setting up signal handlers\n");
#endif
        sa.sa_flags = SA_SIGINFO;
        sigemptyset(&sa.sa_mask);
        sa.sa_sigaction = rf_sigchild_handler;
        if (sigaction(SIGCHLD, &sa, NULL) == -1){
                printf("libexnl.exein_agent_start - can't install the signal handler.");
		return NULL;
                }
...
#ifdef DEBUG
        printf("libexnl.exein_agent_start - allocating memory structures\n");
#endif
...
#ifdef DEBUG
        printf("libexnl.exein_agent_start - starting threads\n");
#endif
...
#ifdef DEBUG
        printf("libexnl.exein_agent_start - sync with threads\n");
#endif
	while (sk_args.loading_done==0) sleep(1);
	while (rf_args.loading_done==0) sleep(1);
#ifdef DEBUG
        printf("libexnl.exein_agent_start - setup done\n");
#endif
	return uhandle;
}
